...
# Piper TTS Service Manager Script
# Provides easy management of the Piper TTS service
...
get_pid() {
    if [ -f "$PID_FILE" ]; then
        ...
        ps aux | grep "python tts_server.py" | grep -v grep | awk '{print $2}'
    fi
}
...
status_service() {
    if is_running; then
        ...
        if command -v curl &> /dev/null; then
            echo ""
            echo "Health Check:"
            curl -s http://localhost:8003/health | python -m json.tool || echo "Service not responding to health check"
        fi
    else
        echo "$SERVICE_NAME is not running."
    fi
...
test_service() {
    if ! is_running; then
        ...
        echo "Please start the service first: $0 start"
        exit 1
    fi
    ...
    if [ -f "test_client.py" ]; then
        ...
        echo "  curl http://localhost:8003/voices"
    fi
}
...
case "${1:-}" in
    ...
    *)
        ...
        echo "  test    - Test the service with sample requests"
        exit 1
        ;;
esac
